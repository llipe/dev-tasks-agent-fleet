...
| S-107 | https://github.com/llipe/dev-tasks-agent-fleet/issues/120 | in issue body — 32 items (2.1–2.31, incl. 2.13a) | https://github.com/llipe/dev-tasks-agent-fleet/issues/120#issuecomment-5542767664 |
...
**Flagged-gap status.** The test plan raised seven gaps. **G1 is resolved** — `/DESIGN.md` v1.1 removes the compact relative-time row from §7.1, so both screens share `formatRelative` and CT-11 is unblocked. **G3 and G4 now have instruments** rather than intentions (tasks 1.16a–c and 2.9/2.16 — see the wave-level note below); the one piece still needing a decision is the **AC-107.7 branch** and whether a filter input is in S-107 scope, both blocking task 2.13. **G5** (AC-108.4's reaper-paused check is manual-only, so it verifies once and then rots — add a stale-row Layer 2.5 test alongside the runbook) is actioned inside task 3.15. **G2** (Layer 2.5 skip-to-green, [#134](https://github.com/llipe/dev-tasks-agent-fleet/issues/134)) is inherited and deferred by decision, but it now covers CT-1/CT-7/CT-8, each the only check on a distinct dashboard failure. **G6** (DESIGN conformance is review-only at screen scale) and **G7** (1024px is unverifiable in jsdom → manual now, Playwright in S-114) are recorded, low-cost mitigations noted in the plan.
...
- **Where the test-plan gaps land.** **G3** and **G4** now carry concrete instruments rather than intentions, added as lettered sub-items so the parent numbering is stable: **G3** → tasks **1.16a/b/c** (a `renderHydrated` helper using `hydrateRoot` + `onRecoverableError`, because RTL's `render()` never hydrates and therefore no current test can observe a mismatch; plus a standing `console.error` trap and a `suppressHydrationWarning` grep). **G4** → task **2.9** and **2.16** for the no-refetch instrument (assert the toggle never navigates, and that the page reads once per render — a client component cannot refetch Supabase at all, so navigation is the only real risk), and task **2.13/2.13a** for the ledger affordances, which stays **blocked on a decision**. **G5** (a stale-row Layer 2.5 test alongside the manual reaper-paused runbook) is actioned inside task **3.15**; **G6** (assert the exact `grid-template-columns` strings) inside tasks **2.6** and **3.6**. **G1** was a `/DESIGN.md` decision and is resolved in v1.1.
...
- [ ] 2.0 Implement Story S-107 ([#120](https://github.com/llipe/dev-tasks-agent-fleet/issues/120)): Agents Dashboard with three-variant density toggle
  ...
  - [ ] 2.13 Implement the ledger keyboard affordances — **blocked on two decisions (test-plan G4); resolve before starting this sub-task.** (a) Do `Up`/`Down`/`Enter` ship, or does the ledger render no keyboard affordances at all? The AC's "or render them absent" must collapse to one branch, because both branches pass a suite that asserts nothing. (b) `/` focuses the filter input, but **no S-107 acceptance criterion requires a filter input** — it appears only in `/DESIGN.md` §5.1's common-elements list, so `/` currently has nothing to focus. Either the filter input is in scope (then `/` is testable as specified) or it is not (then the `/` hint must not render). Record both answers in the PR, then implement exactly one branch
  - [ ] 2.13a Add the hint/behavior coupling guard: if a key's hint text renders, pressing that key must produce its effect. This makes "render the hints, wire nothing" a permanently failing state rather than a passing one, whichever branch 2.13 takes
  ...
  - [ ] 2.16 Run Tests — component: `pnpm run test` — each variant renders from one fixture; persisted value selects the variant on mount; disabled agents excluded. **No-refetch instrument (test-plan G4):** mock `next/navigation`, switch dense → cards → ledger → dense, and assert `push`/`replace`/`refresh` were never called; separately assert `page.tsx` calls the dashboard query exactly once per render with the query module mocked. "No refetch" here reduces to "no navigation and one read per render" — a network spy would assert something the `server-only` boundary already guarantees
