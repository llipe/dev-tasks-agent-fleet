...
- [ ] 2.0 Implement Story S-107 ([#120](https://github.com/llipe/dev-tasks-agent-fleet/issues/120)): Agents Dashboard with three-variant density toggle
  ...
  - [ ] 2.15 Run Tests — unit: `pnpm run test:unit` — aggregation shaper with zero runs, mixed statuses, and a stale `running` row that must land in the `timed_out` bucket; filter predicate (name match, slug match, case-insensitivity, whitespace-only query, no match, regex metacharacters treated literally)
  - [ ] 2.16 Run Tests — component: `pnpm run test` — each variant renders from one fixture; persisted value selects the variant on mount; disabled agents excluded; the filter narrows the list in all three variants and clearing it restores the full list. **No-refetch instrument (test-plan G4):** mock `next/navigation`, switch dense → cards → ledger → dense, and assert `push`/`replace`/`refresh` were never called (assert the same across a filter keystroke); separately assert `page.tsx` calls the dashboard query exactly once per render with the query module mocked. "No refetch" here reduces to "no navigation and one read per render" — a network spy would assert something the `server-only` boundary already guarantees
  - [ ] 2.16a Run Tests — keyboard (component): `pnpm run test` — `Up`/`Down` move the ledger selection and clamp at both ends; `Enter` activates the row's Invoke target; `/` focuses the filter from every variant; `/` typed **inside** the filter inserts a slash rather than re-focusing; `Escape` clears, then blurs when already empty; roving `tabindex` keeps exactly one row focusable
  ...
  - [ ] 2.18 Run Tests — edge cases: zero agents; agent with zero runs; last run `running` (pulse) and last run `failed_to_start` (hollow dot, muted); a stale `running` run displaying `timed_out`; very long description (ellipsis in rows, 2-line clamp in cards); 100+ runs (strip caps at 24); filter matching nothing → the distinct no-match message; ledger `Up`/`Down` at the first and last row; `Enter` with no Invoke target available
  - [ ] 2.19 Manual/UI verification: `pnpm --filter panel dev` → `/` in all three variants at 1024px and 1440px against `docs/prototype/`; reload confirms density persistence **and** confirms the filter did not persist
  ...
  - [ ] 2.25 Verify Acceptance Criterion: empty states render legibly, with no blank region and no `NaN`, and the no-match state reads differently from "no agents configured"
  - [ ] 2.26 Verify Acceptance Criterion: the ledger's keyboard affordances all work — `Up`/`Down` clamped selection, `Enter` on the Invoke target, `/` focusing the filter
  - [ ] 2.26a Verify Acceptance Criterion: the header filter input renders in all three variants, filters by name and slug case-insensitively, never refetches or navigates, and does not survive a reload
  - [ ] 2.27 Map acceptance criteria to test evidence and record the mapping in the PR: AC1/AC4 → integration + component tests; AC2/AC3 → variant + persistence tests + manual reload (AC9); AC5 → link assertions; AC6 → both empty-state tests; AC7 → per-key keyboard tests + the 2.13a coupling guard; AC8 → filter component tests + the no-navigation assertion
