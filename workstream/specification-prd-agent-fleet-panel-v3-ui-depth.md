...
3. **Repository restore — SETTLED during S-148 delivery.** This spec did not add a restore action (an operator needs a direct DB `UPDATE ... SET archived_at = NULL`). That was confirmed acceptable for v1 (no restore UI shipped; recorded as an explicit, confirmed v1 limitation in `DESIGN.md` §5.6 and `docs/technical-guidelines.md` row 1.41), not an oversight.
4. **`/DESIGN.md` §5.6 authorship — SETTLED (did not happen as hoped).** The anticipated `ux-engineer` pass on the Repositories screen layout did **not** land before Story S-147 was implemented; per the story's own documented fallback instruction, `DESIGN.md` §5.6 was written as an explicit first-pass layout composed from existing primitives instead (flagged in-doc for a future design review, not silently improvised).
