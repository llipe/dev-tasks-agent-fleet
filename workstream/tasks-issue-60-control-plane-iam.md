...
- [ ] 1.0 Implement Issue [#60](https://github.com/llipe/dev-tasks-agent-fleet/issues/60): control-plane role needs Fly OIDC trust and CloudWatch Logs grants
  ...
  - [ ] 1.19 Verify Acceptance Criterion 6: **blocked on the TLS cert.** Requires an authenticated session, and the middleware 401s every route without a valid `Cf-Access-Jwt-Assertion`. Cloudflare Access only fronts `fleet.llipe.com`, whose Fly certificate is still `Not verified`. Grey-cloud the DNS record until `fly certs check` reports issued, re-proxy, then load `https://fleet.llipe.com/agents` and check `fly logs` for absence of `AccessDeniedException` on `StartQuery`, `GetQueryResults`, `FilterLogEvents` or `GetResources`
