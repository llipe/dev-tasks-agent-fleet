...
> **Status (2026-09-06): DELIVERED + LIVE-VERIFIED.** Both stories merged to `main` via consolidated PR [#143](https://github.com/llipe/dev-tasks-agent-fleet/pull/143); #125, #126, and #89 closed. `make validate` green on both branches. The three previously-blocked live items are now **closed**: tasks 1.17 (AC1+AC2) and 1.27 (OQ2) were verified against the deployed AgentCore runtime on 2026-09-06 — see [`docs/runbooks/issue-89-live-verification.md`](../docs/runbooks/issue-89-live-verification.md). The only remaining non-`[x]` item is task 2.18's 1024/1440 pixel-geometry comparison, deferred to the S-114 Playwright suite (marked `[~]`).
...
- [x] 1.0 Implement Story S-112 ([#125](https://github.com/llipe/dev-tasks-agent-fleet/issues/125)): Invoke route handler and agent payload translation (closes #89)
  ...
  - [x] 1.17 Manual/live verification (needs the deployed runtime): one real invocation transitioning `queued → running` (#89 AC1), and one deliberately malformed payload confirming the agent's `INVALID_PARAMS` (#89 AC2). **VERIFIED 2026-09-06** against the deployed runtime — AC1 PASS (run_id `a7203345-…`, `running` at 20:12:31 UTC, agent entered the pipeline at 20:12:32); AC2 PASS (session `209f483b-…`, terminal `error_code: INVALID_PARAMS`, CloudWatch `Invalid payload — missing required fields`). Evidence: [`docs/runbooks/issue-89-live-verification.md`](../docs/runbooks/issue-89-live-verification.md)
  ...
  - [x] 1.27 Verify Acceptance Criterion: the `prompt`-wrapping question (OQ2) is settled by observation on the first real integration and recorded — **SETTLED 2026-09-06**: the panel must **not** add a `prompt` wrapper and does not need to; its bare JSON reached `running`, and the `wrapper_only` "appears double-wrapped" diagnostic has never fired (12 h log sweep, zero events; Check B classified `missing_fields`). Recorded in the runbook and `technical-guidelines.md` §8
...
- [x] OQ2 (`prompt` wrapping) — **SETTLED by live observation 2026-09-06** (no longer blocked): the panel sends bare JSON and it is accepted end to end; the "appears double-wrapped" diagnostic has never fired. The `unwrap_payload` tolerance stays relevant for the `agentcore` CLI path (#97), not the panel's SDK path. Evidence in [`docs/runbooks/issue-89-live-verification.md`](../docs/runbooks/issue-89-live-verification.md)
