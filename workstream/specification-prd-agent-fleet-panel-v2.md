...
**Open Question #5 remains open, but no longer as a pre-deploy contract question.** The OIDC socket's response shape and AWS's `sub` claim normalization are still unverified against a real Machine — but the panel **is now deployed** (auth wave / S-123), so this is the **one remaining live-verification item**, not a blocker on the spec. The provider contract is defined and shipped (`panel/lib/aws/credentials.ts`, F5-corrected), a live end-to-end invocation was observed 2026-09-06 (#89), and the socket probe is a documented operator step needing an interactive session on the running Machine (`docs/runbooks/panel-deployment.md` Impl Step 5). Only a probe closes the socket-shape / `sub` / `DurationSeconds ≤ MaxSessionDuration` confirmations; the provider fails loudly (never silently falls through to the local chain) if the live shape differs.
