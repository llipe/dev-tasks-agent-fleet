...
| 1.4     | 2026-09-03 | Wave 2 write-back pass. Four corrections, no scope or decision change; all four are Intended drift recorded by the S-101/S-104 audits and reconciled with explicit user confirmation. (1) **§16 `@supabase/supabase-js` pin corrected `2.58.0` → `2.114.0`** — S-104 re-confirmed the current release and verified it audit-clean before pinning, the same "re-confirm-then-pin" step that produced the v1.3 `next` correction; same major, so the server-side read + Realtime-relay surface is unchanged. (2) **§9.2 and new risk SR9 record that the production service-role read path is proven by inference, not by live assertion** — the canonical migration grants no explicit `SELECT` to `service_role`, and the hosted project works only because Supabase applies platform-default table privileges automatically; `supabase db reset` does not reproduce them, so the Layer 2.5 `queries` test grants them locally (scoped to `service_role` only, RLS deny-all preserved). Mitigation is now owned: a live service-role smoke read was added as an AC on S-115 ([#128](https://github.com/llipe/dev-tasks-agent-fleet/issues/128)). Moving the platform grants into the migration stays explicitly rejected. (3) **SD11 gains the PostgREST `max_rows` interaction** — `max_rows = 1000` in `supabase/config.toml` sits below SD11's 2,000-event cap, so a single `.limit(2000)` would silently return 1,000 rows; `getRunEvents` pages internally with `.range()`. This is how SD11's bound is actually honored, not an implementation aside. (4) **Layer 2.5 skip-to-green recorded as an owned gap** — §14 now names [#134](https://github.com/llipe/dev-tasks-agent-fleet/issues/134) as the mitigation for SR3's parity test and security-negative test 2 being Docker-gated. Companion write-back in [`user-stories-prd-agent-fleet-panel-v2.md`](user-stories-prd-agent-fleet-panel-v2.md) v1.1 (S-103 AC-3 wording). | product-engineer |
| 1.5     | 2026-09-04 | S-111 write-back pass (Wave 2 close-out). Two corrections, no scope or decision change; both are Intended drift recorded by the S-111 audit ([`fidelity-report-S-111.md`](fidelity-report-S-111.md)) and reconciled at the user's instruction. (1) **§16 `@aws-sdk/*` pinned and the "same minor" requirement corrected to its achievable form** — the three clients are `3.1126.0` and `@aws-sdk/types` is `3.974.5`; `types` tracks an independent, slower cadence, so a shared minor across all four was never satisfiable. The requirement's real purpose was avoiding duplicate `@smithy/core` copies, so the invariant is restated as "three clients on one minor + exactly one `@smithy/core` in the lockfile" (S-111 shipped `@smithy/core@3.33.3`). Left as-written, the plan checkbox asserting it would be perpetually literally-false. (2) **SD9 records the role-ARN env var as `AGENT_RUNTIME_ROLE_ARN`** (was `AWS_ROLE_ARN` in the reference stub), read with a fail-fast named startup error; renamed away from the `AWS_*` namespace the SDKs read themselves. **Also written back in this pass:** the three `/DESIGN.md` self-contradictions from the S-105 audit are resolved in `/DESIGN.md` **v1.1** with explicit user go-ahead — D1 (uniform 14% pill tint, §3.4 was the authority), D2 (1.6s pulse everywhere including `queued`), and D3 (the compact `14m ago` row is **removed** from §7.1, so one relative-time form is shared by the dashboard and the run history table). All three match what S-105 already shipped, so no code changes; D3 was decided before Wave 3 started because it was S-107 scope. | product-engineer |
| 1.6     | 2026-09-04 | Wave 3 pre-implementation pass, resolving test-plan gap **G4** with explicit user decisions. One §10 change, no decision reversal. **The Agents Dashboard row now records a header filter input and the ledger keyboard affordances.** The filter is client-side over the already-loaded list (name + slug, case-insensitive), rendered in all three variants, and **transient by design** — density persists, the filter does not, because a remembered filter would hide agents on the next load with no visible cause. `/` focuses it (a deliberate superset of `/DESIGN.md` §6.5's ledger-only binding, since the input is a common header element) and `Escape` clears then blurs. The ledger affordances ship unconditionally — `Up`/`Down` clamped with roving `tabindex`, `Enter` activating the same target as the row's Invoke action so the keyboard path cannot reach a route the mouse path deliberately does not. This closes the gap where AC-107.7's "**or** renders them absent" made the criterion unfalsifiable and `/` had nothing to focus. Companion write-back in [`user-stories-prd-agent-fleet-panel-v2.md`](user-stories-prd-agent-fleet-panel-v2.md) v1.2 (AC-107.7 rewritten, AC-107.8 added). **Not a PRD change** — FR10 already requires the agent list and this is presentation of it; PRD §10's deferral of **run history** filtering is untouched and S-108 still ships no filters. | product-engineer |
| 1.7     | 2026-09-11 | Drift-reconciliation write-back after the auth wave shipped and the panel went public (S-116…S-123, issue #162). No decision made *here* — the auth decisions live in [`specification-panel-password-auth.md`](specification-panel-password-auth.md); this pass reconciles the now-stale v2 records. **§7 SD7 annotated SUPERSEDED** (historical text preserved): "no user auth; the boundary is the Fly app's privacy" no longer holds — the panel has a Supabase password login behind a fail-closed `middleware.ts` chokepoint and is deployed public with **login as the boundary**, mechanically gated ([ADR-007](../docs/adr/ADR-007-auth-release-gate-replaces-privacy-gate.md)); SA1 adds a second anon-key cookie client family, the service-role data client is untouched, RLS stays deny-all (D11). **§9.1** `prompt`-wrapping question marked **RESOLVED** (bare JSON, #89, 2026-09-06) and the "Open Question #5 blocks AC8" note marked **RESOLVED** (2026-09-11 — Fly OIDC → AWS STS confirmed working on the deployed Machine, no static keys, live invocation; closes the live half of AC8), no longer a spec blocker. Note SD6 (§6.2) already fully specifies gap-free SSE reconnect; the auth-consequence "stop reconnecting on a pre-open 401" (S-121) is an auth-spec concern (its §6.3/§8.4), not a v2-spec open question. Companion write-backs: PRD v2.5 (D16-reversed/R1-resolved) and `DESIGN.md` §5.1/§8.2. Source: `workstream/drift-reconciliation-2026-09-11-auth-go-public.md`. | product-engineer |
...
> **SUPERSEDED (2026-09-11, auth wave S-116…S-123).** SD7 no longer describes the shipped
> system. D16 was **reversed**: the panel now has *operator → panel* authentication — a Supabase
> email + password login behind a fail-closed `panel/middleware.ts` gate (the single authorization
> chokepoint; `getClaims()`, `/login`, POST logout, SSE `401`), and it is **deployed public** over
> HTTPS with **login — not the Fly app's privacy — as the security boundary**. The design lives in
> [`specification-panel-password-auth.md`](specification-panel-password-auth.md) (the second,
> independent anon-key cookie client family SA1; the service-role data client is untouched, RLS stays
> deny-all D11). The boundary is mechanically asserted by the **auth release gate**
> (`scripts/verify-panel-auth.sh` + `panel/scripts/panel-auth-check.mjs`), which replaced the S-115
> privacy gate ([ADR-007](../docs/adr/ADR-007-auth-release-gate-replaces-privacy-gate.md)). The
> table and the "stated plainly to whoever deploys" warning below are retained as the historical
> Phase-2 record; the current permission model is single-role (anonymous → 302 /login or 401;
> authenticated → full access), not a single anonymous principal.
...
> **RESOLVED (2026-09-06, [issue #89](https://github.com/llipe/dev-tasks-agent-fleet/issues/89)).** Confirmed by observation against the deployed runtime: the panel sends the **bare inner JSON** (`panel/lib/aws/invoke.ts`, **no** `prompt` wrapper) and it is accepted end-to-end (a bare payload reached `running`; a malformed bare payload was rejected `INVALID_PARAMS`). The `prompt`-envelope requirement is specific to the `agentcore` **CLI** path (the CLI wraps its argument itself, which is why the CLI double-wrap of #97 exists); the panel's SDK path needs no wrapper and must not add one. Evidence: [`docs/runbooks/issue-89-live-verification.md`](../docs/runbooks/issue-89-live-verification.md).
...
**Role-ARN env var is `AGENT_RUNTIME_ROLE_ARN` (recorded v1.5, from S-111).** The reference stub read the role ARN from `AWS_ROLE_ARN`; the adopted module reads `AGENT_RUNTIME_ROLE_ARN` and fails fast with a named startup error when it is unset, rather than surfacing an opaque invoke-time auth failure. The rename is deliberate — `AWS_*` is a namespace the AWS SDKs themselves read, so a panel-specific variable cannot be confused with (or silently shadowed by) SDK-native configuration. `.env.example` carries it.

**Open Question #5 is RESOLVED (2026-09-11) — the panel has a working Fly OIDC → AWS STS connection.** The two-branch credential provider (`panel/lib/aws/credentials.ts`, F5-corrected) is confirmed working live: on the deployed Fly Machine the panel obtains an OIDC token from the local socket and exchanges it via `AssumeRoleWithWebIdentity` for short-lived AWS credentials with **no static keys** (D12), and a live agent invocation reaches AgentCore over that path (`credentialSource(): "fly-oidc"`). The socket-response shape and AWS's `sub` claim normalization matched the SD9 contract (the provider is written to fail loudly rather than silently fall through to the local chain, so a mismatch would have surfaced as `FlyOidcShapeError` / an assume-role failure, not a false pass). This closes AC8's live half. Operator-confirmed; the probe procedure remains recorded in `docs/runbooks/panel-deployment.md` (Impl Step 5) for re-verification.
...
**The production read path is proven by inference, not by live assertion (recorded v1.4, from S-104).** The canonical migration grants **no** explicit `SELECT` to `service_role`. Reads work against the hosted project only because Supabase applies platform-default table privileges to `service_role` automatically — confirmed by the S-115 baseline diff (`docs/runbooks/issue-115-baseline-adoption.md`), not by an assertion this spec's test suite makes. A fresh local database from `supabase db reset` does **not** reproduce those grants, so the Layer 2.5 `queries` test applies the grant in setup, scoped to `service_role` **only**; `anon` is never granted, so RLS deny-all (D11) and security-negative test 2 remain intact.

Keeping the platform-managed grants **out** of the migration is deliberate and stays that way — moving them in is the platform-tampering rejected during S-115 baseline adoption. The honest consequence: if a platform change or a from-scratch reprovision ever stripped those defaults in production, the whole suite would stay green while production reads returned `42501`. Recorded as **SR9**, mitigated by a live service-role smoke read added as an AC on S-115 ([#128](https://github.com/llipe/dev-tasks-agent-fleet/issues/128)).

**PostgREST `max_rows` bounds every read below SD11's cap.** `supabase/config.toml` sets `max_rows = 1000`, so any single request returns at most 1,000 rows regardless of the requested limit — silently, with no error. Read helpers whose contract exceeds that ceiling must page internally; see SD11.
...
| Agents Dashboard | `/` | FR10, FR17 | Three density variants behind a toggle, default dense rows, `localStorage` persisted (D17). Header filter input over the already-loaded list (client-side, name + slug, **transient** — v1.6, from S-107 AC8); `/` focuses it, `Escape` clears then blurs. Ledger keyboard affordances ship: `Up`/`Down` clamped roving-`tabindex` selection, `Enter` on the row's Invoke target |
...
**PostgREST `max_rows` makes the 2,000-event read a paged read (recorded v1.4, from S-104).** `supabase/config.toml` sets `max_rows = 1000`, below this cap. A single `.limit(2000)` therefore returns 1,000 rows and reports no error — the failure is silent truncation, which on a log viewer looks like missing history rather than a bug. `getRunEvents` consequently pages internally with `.range()` in chunks of at most 1,000 (newest-first by `seq`, re-sorted ascending for display) until it reaches the cap or exhausts the stream. This is how the SD11 bound is honored; it is not an optional optimization, and any future helper whose contract exceeds `max_rows` inherits the same requirement. Changing platform config to raise `max_rows` is not the chosen answer — the ceiling is a useful backstop against exactly the unbounded fetch this decision exists to prevent.
...
**Layer 2.5 is Docker-gated, so its two highest-value tests can pass vacuously (recorded v1.4, from S-104).** The parity test above (SR3's only mitigation) and security-negative test 2 (the F2 test) both skip with a recorded reason when the local Supabase stack is unreachable, which is correct on a developer machine and wrong in CI — a green run there does not distinguish "proven" from "never checked." They ran live for S-104 only because the operator started the stack by hand. Making a skipped parity or deny-all project **fail** CI is [#134](https://github.com/llipe/dev-tasks-agent-fleet/issues/134), and until it lands every story adding a Layer 2.5 suite must state whether the suite ran live or skipped. A skip is not evidence.
...
| `@supabase/supabase-js` | `2.114.0` | Server-side reads + Realtime relay. **Corrected from `2.58.0` in v1.4** — S-104 re-confirmed the current release and verified it audit-clean before pinning, per the re-confirm-then-pin rule below. Same major; no surface change |
| `server-only` | `0.0.1` | Build-time guard on `lib/supabase/server.ts` — pulling it into a client bundle fails `next build` with an RSC error no `eslint-disable` can suppress. This, not the lint rule, is what makes SD2 enforceable (S-104) |
| `@aws-sdk/client-bedrock-agentcore` | `3.1126.0` | `InvokeAgentRuntime`. **Pinned in v1.5 (S-111)** |
| `@aws-sdk/client-sts` | `3.1126.0` | `AssumeRoleWithWebIdentity` (already a `credentials.ts` dep). **Pinned in v1.5 (S-111)** |
| `@aws-sdk/credential-providers` | `3.1126.0` | Local chain branch. **Pinned in v1.5 (S-111)** |
| `@aws-sdk/types` | `3.974.5` | `credentials.ts` type imports. **Pinned in v1.5 (S-111)** — independent, slower release cadence than the three clients, so it does **not** share their minor; see the corrected invariant below |
...
Versions must be confirmed current at implementation time. The remaining pins are best-known-current and must be re-confirmed rather than trusted.

**The `@aws-sdk/*` "same minor" requirement is corrected to its achievable form (v1.5, from the S-111 audit's D1 finding).** The original wording — all four packages pinned to the same minor — is not satisfiable: `@aws-sdk/types` tracks an independent, slower release cadence than the clients, so no single minor covers all four. The requirement's actual purpose was never version symmetry; it was avoiding duplicate transitive copies of `@smithy/core` in the lockfile. The enforceable invariant is therefore: **the three clients (`client-sts`, `client-bedrock-agentcore`, `credential-providers`) share one minor, and `pnpm-lock.yaml` holds exactly one `@smithy/core` version.** S-111 satisfied both (`3.1126.0` across the clients, a single `@smithy/core@3.33.3`). Verify the deduplication, not the version strings.
...
| SR3 | SD4's duplicated status logic drifts from the SQL view | Medium | Layer 2.5 parity test (§14). **The test is Docker-gated and skips to green when the local stack is down** — making that skip fail CI is [#134](https://github.com/llipe/dev-tasks-agent-fleet/issues/134) (recorded v1.4) |
...
| SR9 | Production `service_role` SELECT rests on platform-default grants the migration does not declare and the test suite does not assert (§9.2) | Low | Live service-role smoke read after deploy — AC added to S-115 ([#128](https://github.com/llipe/dev-tasks-agent-fleet/issues/128)). Do **not** move platform grants into the migration (recorded v1.4) |
