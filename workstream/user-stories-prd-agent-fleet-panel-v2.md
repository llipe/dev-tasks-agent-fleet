...
- [ ] The seed's Spanish comments and block headers are translated; no Spanish remains in `supabase/seed.sql` or the migrations directory (`grep` evidence in the PR).
...
- **Acceptance-Criteria Mapping:** AC1, AC6 → `pnpm run test:integration` reaper cases; AC2–AC3 → integration assertion + `grep -nP "[^\x00-\x7F]" supabase/` empty; AC4 → doc diff + PR rationale; AC5 → `make validate` Python branch.
