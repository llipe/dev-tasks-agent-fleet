...
| OQ1 | **Spec** §17 / **PRD** Open Question #5 (Fly OIDC socket shape + normalized `sub` + `MaxSessionDuration`) | Open — "blocks AC #8" | **Still OPEN** — genuinely not yet verified. The live OIDC socket probe needs a live Machine (runbook Impl Step 5). Leave open; note it is the one remaining live-verification item, now that the app is deployed. | runbook `panel-deployment.md` Impl Step 5; §13 row 1.30 |
