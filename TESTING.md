...
- Baseline: **~90%+ on implemented modules** — `scrubber.py` 100%, `config.py` 100%, `classifier.py` 100%, `eligibility.py` 100%, `credentials.py` 100%, `pull_request.py` 95%, `validator.py` 99%, `toolchain.py` 94%, `fix_agent.py` ~91%, `audit.py` ~87%. `main.py` and `agent_reporter.py` remain coverage-excluded/untested (see gap table). This baseline now describes the bulk of the pipeline logic, not just a narrow slice.
...
| ---------------------------------- | ------- | ---- | ---- |
| `credentials.py`                   | partial | HIGH | GitHub App RS256 JWT auth + Supabase key resolution. Happy paths, staleness, transport-failure classification (`SUPABASE_UNREACHABLE`/`GITHUB_UNREACHABLE`, incl. explicit `Timeout`), and Secrets Manager failure classification (`SUPABASE_KEY_UNAVAILABLE`/`PEM_UNAVAILABLE` on `ClientError` + missing `SecretString`) all covered (**100%**, #106/#108/#109). Residual security-negative gap: GitHub-server-side JWT signature/expiry/issuer/tamper *rejection* is unexercised because the token endpoint is mocked (see Security-Negative §). |
...
>
> **What genuinely IS covered (positive-adjacent + boundary-failure classification):** the `NO_INSTALLATION` error when no enabled row matches; the client-side token-staleness/refresh boundary logic; and — as of #106/#108/#109 — the **boundary-failure classification** on both external surfaces: a `requests.RequestException` (incl. `Timeout`) reaching Supabase/GitHub surfaces as `SUPABASE_UNREACHABLE`/`GITHUB_UNREACHABLE`, and a Secrets Manager `ClientError` or missing `SecretString` surfaces as `SUPABASE_KEY_UNAVAILABLE`/`PEM_UNAVAILABLE` (the new `TestFetchPem` class closed the prior `_fetch_pem` zero-coverage gap; `credentials.py` is now **100%**). These are real and useful — they prove a failed boundary is *classified rather than raised raw* — but they are still not security-negative *rejections* of bad credentials.
>
> **Recommended (hand to `developer` — requires no new deps):** ~~(a) feed a `requests.post` mock returning 401/403 from the token endpoint and assert `mint_installation_token` raises rather than returns a token~~ — the transport/Secrets-Manager failure-classification half of (a) is now covered (#106/#108); the remaining work is the server-side *rejection* cases: (b) assert `_get_installation` never returns a row for a mismatched or disabled org; (c) once a real signature check is reachable (e.g. verifying the assertion with the public key in a component test), assert tampered/expired/wrong-issuer assertions are rejected. Until a real policy or verification layer is exercised, this section MUST remain marked as a gap, not satisfied — classifying a *transport/secret-store failure* is not the same as a server *refusing bad credentials*.
