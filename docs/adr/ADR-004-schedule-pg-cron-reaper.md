...
4. **Route the defects the verification surfaced to their own issues** rather than
   ...
   defects live elsewhere in the stack (#97, #98, #99, #100).
...
- **Two acceptance criteria remain pending.** AC5 (healthy long-running
  `llm_fix` not reaped early, plus the cold-start gap measurement) is blocked by
  #98, since no long run can currently complete; the synthetic interlock proof in
  the runbook §4.4 is the recommended substitute. AC6 (CloudWatch fallback) was
  not executed. Both are recorded as `PENDING` in the runbook's verification
  status summary rather than silently omitted.
- **The cold-start gap remains unmeasured.** The one attempt
  ...
  atomically is Phase 2. dependency-update PRD open question 8 stays open.
...
- `docs/technical-guidelines.md` §3 (two-layer design), §7 (thresholds), §18 (open defects)
- Parent PRD `prd-agent-fleet-panel-v2.md` — FR3, FR8, FR11a, D8, D9, D10
- Follow-ups: [#97](https://github.com/llipe/dev-tasks-agent-fleet/issues/97), [#98](https://github.com/llipe/dev-tasks-agent-fleet/issues/98), [#99](https://github.com/llipe/dev-tasks-agent-fleet/issues/99), [#100](https://github.com/llipe/dev-tasks-agent-fleet/issues/100)
