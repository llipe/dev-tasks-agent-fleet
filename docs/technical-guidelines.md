...
| Concern      | Choice                                                                 |
| ------------ | ---------------------------------------------------------------------- |
| Language     | **Python 3.12**, where the AgentCore SDK ecosystem lives                |
...
| IaC         | **Terraform**. Remote state with locking. The write-separation IAM policy and the discovery tag scheme are both declarative-shaped. |
...
| `stale` derived at read time                | Nobody can write "I died." A read-time derivation from `last_status = running` and a stale `last_run_at` catches it. |
...
- `last_status` persists only `running`, `success`, or `failed`. **`stale` is never written** — it is derived at read time from `last_status = running` and `last_run_at` older than 6 hours. The threshold is a named constant in `packages/shared`.
...
│       ├── src/lib/              # cost estimation, formatting, stale derivation
...
└── infra/
    ...
    └── agents/                   # Terraform: table, GSI1, scheduler, orchestrator, IAM, tags
```
...
- **Pure mappers.** Span-to-`Run`, cost estimation, and stale derivation are pure functions of their inputs. These hold the logic most likely to be subtly wrong, so they must be testable without AWS.
...
| Layer                | Tool                        | Scope                                                             |
| -------------------- | --------------------------- | ----------------------------------------------------------------- |
| Unit (TS)            | Vitest                      | Mappers, cost estimation, stale derivation, key builders, `params` validation |
...
- `stale` derivation at the 6-hour boundary, and the distinction between `stale` and a genuinely running run.
...
- Comments explain *why*, not *what*. The non-obvious decisions here — orchestrator-generated `session_id`, read-time `stale`, agent `UpdateExpression`-only writes — each deserve a comment pointing at the PRD section, because all three look like mistakes to someone who doesn't know the constraint.
...
**Infrastructure.** Terraform with remote state and locking. `plan` on PR, `apply` gated on human approval. Everything in PRD §8, §9, §15 is Terraform-managed: table and GSI1, EventBridge Scheduler rules, the orchestrator Lambda, IAM roles including the `dynamodb:Attributes` constraint, discovery tags, and the OIDC trust policy.
...
| `infra/**`               | `terraform plan` → gated `apply`            |
...
**Alerting: none in v1.** Out of scope, and honestly so. The `stale` status is the manual substitute: it turns "an agent died quietly" into something visible on the next look. Nothing pages anyone. Missing-run detection against declared schedules is in the backlog.
...
**Planning artifacts.** New PRDs go in `docs/requirements/`; specifications, user stories, and task lists in `/workstream/`. The v1 PRD sits at `docs/PRD-agent-control-plane-v1-en.md` because it predates this convention — leave it there rather than breaking the links to it. GitHub Issues and PRs are the source of truth for execution status.
...
| Fire-and-forget invocation                                       | No completion signal except the agent's own write. An agent that dies before writing is only detectable via `stale`. |
| 6-hour `stale` threshold                                         | A legitimate run exceeding 6 hours is mislabelled. Tuning the constant is the remedy.                    |
