...
| `panel/` front-end | Next.js 15 (App Router) package under `panel/`, the primary member of the repo-root pnpm workspace — scaffolded in Phase 2 (issue #114 / S-101). Placeholder `app/layout.tsx` (DESIGN §1.2 Inter `<link>`) + `app/page.tsx`. As of issue #117 / S-104 the **server-side Supabase read boundary** exists: `lib/supabase/server.ts` (`createServerClient` per-request factory + `readSupabaseEnv` fail-fast validation, `import "server-only"` hard guard), `lib/supabase/queries.ts` (eight typed read helpers; `getRunEvents` pages internally with `.range()` to honor the SD11 2000-event cap under PostgREST `max_rows=1000` — see §7), `lib/supabase/types.ts` (hand-written row types for the six read objects), `lib/supabase/errors.ts` (`DATABASE_ERROR` — Postgres code logged, never returned to the client), `lib/supabase/route-config.ts` (canonical `force-dynamic`/`revalidate=0`/`fetchCache` values), and `lib/domain/status.ts` (`effectiveStatus`, the pure TS mirror of `v_runs.effective_status` (SD4), pinned to the SQL by a live Layer 2.5 parity test). `app/dev/agent-count/page.tsx` is a manual-verification server route. Routes, schema-generated forms, and live tail land in later Phase 2 stories. React 19, TypeScript strict, ESLint + Prettier, Vitest (unit/component/integration projects) + Playwright config stub. |
