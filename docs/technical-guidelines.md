...
- **Pending verification against the real Fly endpoint** (contract-defined, not yet live-probed — deferred to S-115 / OQ1): the exact JSON response shape from the OIDC socket, the literal claim name `sub` as normalized by AWS in the trust policy, and that `DurationSeconds: 900` is compatible with the role's `MaxSessionDuration`. The provider is written to fail loudly (never a silent fall-through to the local chain) if the live shape differs.
