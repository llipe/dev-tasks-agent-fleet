...
client-component tree (`components/**`) — it was rescoped from tree-wide because the prior scope
flagged legitimate Server Components (exactly where SD2 reads belong) during `next build`. The
guarantee that matters — the service-role key never enters the browser — is carried by
`server-only`, so the rescope is strictly stronger, not weaker; `tests/unit/eslint-server-import.test.ts`
