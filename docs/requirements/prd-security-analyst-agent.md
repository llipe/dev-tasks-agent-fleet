...
| 1.3     | 2026-09-17 | Status notes only, no requirement changes: §5/§12's "no schema migration required" claim is annotated as superseded by the additive `run_outcome` enum migration (`no_findings`) that S-141's real invocation required; §14's panel-rendering consequence annotated as shipped (#241/#243). | technical-writer |
...
| Supabase project (infrastructure)       | Consumes writes to `runs`, `run_steps`, `run_events`, `run_artifacts`. Reuses the existing `github_installations` row — no new installation required. **No schema migration required.** *(Status note, 2026-09-17: one enum migration was in fact needed — see §12 note.)* |
...
*(Status note, 2026-09-17 — claim superseded at implementation time: S-141's first real `audit_only` invocation found the baseline `run_outcome` enum has `no_vulnerabilities` but not this PRD's `no_findings` (used in §8.1, AC3, AC12b, AC21), so every clean run failed to PATCH `runs` and stuck in `running`. The additive migration `supabase/migrations/20260916194500_add_no_findings_run_outcome.sql` (`alter type run_outcome add value if not exists 'no_findings'`) is the one schema change this agent required; no tables were added. The sentence above is left as written for historical accuracy of the v1.2 design intent.)*
