...
| 2.5     | 2026-09-11 | Drift-reconciliation write-back after the auth wave shipped and the panel went public (S-116…S-123, issue #162). **D16 annotated REVERSED** and **R1 annotated RESOLVED** in place (historical text preserved, superseding notes appended): the panel now requires a Supabase password login behind a fail-closed middleware gate and is deployed **public** over HTTPS with **login — not network privacy — as the security boundary**, mechanically asserted by the auth release gate ([ADR-007](../adr/ADR-007-auth-release-gate-replaces-privacy-gate.md); `technical-guidelines.md` §5/§6/§13). §18 open questions updated: the Fly OIDC socket-shape / `sub` / `MaxSessionDuration` question is **now RESOLVED** (2026-09-11 — the deployed panel's Fly OIDC → AWS STS connection is confirmed working, no static keys, live invocation over `credentialSource(): fly-oidc`; closes the live half of AC8), and the panel→agent `prompt`-wrapping question is marked **resolved** (bare JSON, #89, 2026-09-06). New residual risk **R8** (public-login brute force, accepted with Supabase rate limits) is recorded in `technical-guidelines.md` §18. No requirement (FR) or scope change — decision-record status only. Companion write-backs in the same pass: v2 spec SD7/§17 (drift item D2/OQ1/OQ2/OQ3) and `DESIGN.md` §5.1/§8.2. Source: `workstream/drift-reconciliation-2026-09-11-auth-go-public.md`. | product-engineer |
...
| 2.4     | 2026-09-09 | Auth is now split into two iterations: §10's backlog row re-points to [`prd-panel-password-auth.md`](prd-panel-password-auth.md) (email + password, shipping now — retires R1, reverses D16, keeps RLS deny-all D11) with [`prd-panel-auth-and-rls.md`](prd-panel-auth-and-rls.md) recorded as **DEFERRED, not cancelled**. Corrected the F2 hardening note: FR17/FR18 (`REVOKE`, `security_invoker`) do **not** land with the auth work that is shipping, so the standing "Phase 2 must not grant `anon` access" obligation is now indefinite rather than temporary. Clarified that the panel becoming public does not widen those two traps, since the Supabase Data API is internet-reachable independently of Fly networking. | product-engineer |
...
- **D16 — Phase 2 ships with no user authentication, and no shared-secret header either.** The two auth concerns in this system are distinct and must not be conflated: *panel → AWS* machine auth is solved by D12 (Fly OIDC + `AssumeRoleWithWebIdentity`) and is required for invocation to work at all; *operator → panel* user auth does not exist. The alternative mitigation floated in R1 (shared-secret header on the invoke route) is **rejected for this iteration** — it would add an auth layer to the API contract for a benefit that keeping the Fly app private achieves with zero code. Consequence for the spec: no middleware, no session handling, no protected routes. `runs.triggered_by` is populated with a constant or left null, not with an authenticated identity.
  > **REVERSED (2026-09-11, auth wave S-116…S-123).** This decision no longer holds. The panel now has *operator → panel* user authentication: a Supabase email + password login behind a fail-closed `middleware.ts` gate (`/login`, POST logout, SSE `401`), and it is **deployed public** over HTTPS with **login — not network privacy — as the security boundary**, mechanically asserted by the auth release gate (`scripts/verify-panel-auth.sh`). The reversal was specified in [`prd-panel-password-auth.md`](prd-panel-password-auth.md) and the enforceable-rule change recorded in [ADR-007](../adr/ADR-007-auth-release-gate-replaces-privacy-gate.md) / `technical-guidelines.md` §5/§6/§13. The "no middleware / no protected routes / `triggered_by` constant" consequences above are superseded (middleware is now the chokepoint). RLS stays deny-all (D11) — authenticating a user is a process gate, not a row grant. The deferred role-based direction remains [`prd-panel-auth-and-rls.md`](prd-panel-auth-and-rls.md) (GitHub OAuth, `app_users`, per-role RLS), DEFERRED not cancelled.
...
| Item | Future form |
|---|---|
| Supabase Auth with allowlist | **Split into two iterations (2026-09-09).** *Being implemented now:* [`prd-panel-password-auth.md`](prd-panel-password-auth.md) — email + password sign-in, cookie sessions, a middleware gate over all routes, and public Fly exposure behind that gate. Retires **R1** and reverses **D16**; keeps RLS deny-all (**D11**) unchanged. *Deferred future direction:* [`prd-panel-auth-and-rls.md`](prd-panel-auth-and-rls.md) — GitHub OAuth, `app_users` allowlist, `viewer`/`operator` roles, RLS policies per role, `triggered_by` = `auth.uid()`, SSE-relay removal. That PRD is **DEFERRED, not cancelled** — see its §0 for the relationship and its stale assumptions |
...
> **RESOLVED (2026-09-11, auth wave S-116…S-123).** The exit path was taken: the panel now requires a Supabase password login (fail-closed middleware gate, `/login`, POST logout, SSE `401`), so invocation is no longer reachable without a session. The app is deployed **public** over HTTPS with **login as the security boundary**, mechanically asserted by the auth release gate (`scripts/verify-panel-auth.sh` + `panel/scripts/panel-auth-check.mjs`), which fails the release unless unauthenticated requests are denied (protected UI → 302 /login, SSE → 401) **and public signups are rejected** — the gate ran against the public host and passed on 2026-09-11 (see `technical-guidelines.md` §5/§6/§13, [ADR-007](../adr/ADR-007-auth-release-gate-replaces-privacy-gate.md)). The former "keep the Fly app private" mitigation is superseded for the login boundary. **New residual risk (R8, tracked in `technical-guidelines.md` §18):** a public login endpoint invites credential-stuffing / brute force — accepted for v1 relying on Supabase's built-in rate limits, with the signup-disabled release blocker preventing self-registration.
...
- ~~The exact JSON response shape from the Fly OIDC socket, and the literal `sub` claim name as normalized by AWS, remain pending empirical verification against a real Machine~~ → **RESOLVED (2026-09-11).** The panel has a working **Fly OIDC → AWS STS** connection: on the deployed Machine it obtains an OIDC token from the local socket and exchanges it via `AssumeRoleWithWebIdentity` for short-lived credentials with **no static keys** (D12), and a live agent invocation reaches AgentCore over that path (`credentialSource(): "fly-oidc"`). This closes the live half of **acceptance criterion #8**. The socket shape and normalized `sub` matched the shipped provider contract (`panel/lib/aws/credentials.ts`); the provider fails loudly on a mismatch, so this is a positive confirmation, not an unverified assumption. Probe procedure retained for re-verification in `docs/runbooks/panel-deployment.md` (Impl Step 5).
- ~~Does the panel wrap the agent invocation payload in a `prompt` envelope?~~ → **Resolved 2026-09-06** ([issue #89](https://github.com/llipe/dev-tasks-agent-fleet/issues/89)): the panel sends the **bare inner JSON** (`panel/lib/aws/invoke.ts`, no `prompt` wrapper) and it is accepted end-to-end against the deployed runtime; a malformed bare payload is rejected `INVALID_PARAMS`. The agent's repeated-unwrap tolerance remains relevant only to the `agentcore` CLI path (#97), not the panel. Evidence: [`runbooks/issue-89-live-verification.md`](../runbooks/issue-89-live-verification.md).
...
> **Correction (2026-09-09).** "Until the auth work lands" no longer describes the plan. The auth work now landing is [`prd-panel-password-auth.md`](prd-panel-password-auth.md), which **does not** include FR17/FR18 — it keeps deny-all exactly as-is and adds no policies. `prd-panel-auth-and-rls.md`, which carries both fixes, is **DEFERRED**. So the two hardening items stay unlanded for longer than this paragraph implies, and the standing obligation above — *Phase 2 must not grant `anon` access to any table or view* — remains the only thing keeping both traps closed. It applies indefinitely, not just through Phase 2.
>
> This is **not** made worse by the panel becoming publicly reachable (password-auth S-123): the Supabase Data API is reachable from the internet regardless of the Fly app's networking, so the exposure surface for these two traps is unchanged. The risk is the same as before, held closed by the same single control — it simply persists longer.
