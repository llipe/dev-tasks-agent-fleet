...
- The exact JSON response shape from the Fly OIDC socket, and the literal `sub` claim name as normalized by AWS, remain pending empirical verification against a real Machine (§12). The panel **is now deployed** (auth wave / S-123), so this is no longer a pre-deploy contract question — it is the **one remaining live-verification item**: the OIDC socket probe needs an interactive session on the running Machine (`docs/runbooks/panel-deployment.md` Impl Step 5). Acceptance criterion #8 (no static keys, live invocation) is otherwise exercised — a live end-to-end invocation was observed 2026-09-06 ([issue #89](https://github.com/llipe/dev-tasks-agent-fleet/issues/89)) — but the socket-shape / `sub` / `DurationSeconds ≤ MaxSessionDuration` confirmations stay open until the operator runs the probe. The provider is written to fail loudly (never silently fall through to the local chain) if the live shape differs.
