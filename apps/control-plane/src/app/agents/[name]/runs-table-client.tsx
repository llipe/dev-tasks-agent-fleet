...
/**
 * Runs table client component — S-020.
 *
 * Renders the DataTable with run columns, filter controls,
 * and row click handling for the run panel (S-021).
 */
...
import { RunFilters } from "./run-filters-controls.js";
...
export function RunsTableClient({ agentName, runs, state, filters }: RunsTableClientProps) {
  ...
  const handleRowClick = useCallback(
    (row: MergedRun) => {
      ...
      router.replace(`/agents/${encodeURIComponent(agentName)}?${params.toString()}`);
    },
  ...
  return (
    <div>
      <RunFilters agentName={agentName} filters={filters} />
